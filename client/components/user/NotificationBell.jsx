...
import { subscribeToUserNotifications } from "../../lib/userSocket";

// ======================================================
// USER NOTIFICATION BELL (real, wired to backend)
// ======================================================
...
export default function NotificationBell() {
    ...
    const loadNotifications = async () => {
        try {
            setLoading(true);
            const response = await getMyNotifications({ page: 1, limit: 20 });
            if (response?.notifications) {
                setNotifications(response.notifications);
                setUnreadCount(response.unreadCount || 0);
            }
        } catch (err) {
            // Silent - the bell just stays as-is on failure.
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="relative" ref={containerRef}>
            ...
            >
                ...
                {unreadCount > 0 && (
                    <span className="absolute right-1.5 top-1.5 flex h-2 w-2 items-center justify-center rounded-full bg-violet-400 shadow-[0_0_8px_rgba(167,139,250,0.8)]" />
                )}
            </button>
